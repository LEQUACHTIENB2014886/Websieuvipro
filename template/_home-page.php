...
?>
<form id="myform" method="post" action="./select/chap.php">
				...
				<?php
					if ($count1 % 4 == 0) {	
						...
						while ($row = $stmt8->fetch()) {
							...
							echo '<a href="./select/chap.php" data-linktruyen="' . $row['linktruyen'] . '" class="truyen-link"><img src="data:image/jpg;base64,' . base64_encode($row['anh']) . '" width="1000%"></a>';
							echo '<div class="aaa text-center"><b><a href="./select/chap.php" class="text-info truyen-link" data-linktruyen="' . $row['linktruyen'] . '">' . $row['tentruyen'] . '</a></b></div>';
							echo '</div>';
							$count1++;
						}
						echo '</div>';	
